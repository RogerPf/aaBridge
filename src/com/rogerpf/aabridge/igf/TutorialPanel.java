...
//	/**
//	 */
//	public void action_su(GraInfo gi) { // special Use clear
//		// =============================================================================
//		if (App.alwaysShowHidden && (App.su_clear_overide == false)) {
//			App.alwaysShowHidden = false;
//			App.frame.rop.p2_KibSeat.do_compassClear_and_showAlwaysHiddenClear();
//		}
//	}
...
//	final float SYMBOL_YPOS_ADJUST = 0.2f;
			...
			if (seg.hyperlink != null) {
				for (Ras ras : seg) {
					...
					if (contains) {
						...
						seg.hyperlink.actionLink(ctrlKey_depressed);
					}
				}
			}
		}
	...
	public void paintComponent(Graphics g) {
		// =============================================================================
		if (App.mg == null)
			return; // Can it really happen that TutorialGraphics is null ? !
		...
		for (int i = mg.start_nt; i <= mg.end_pg; i++) {
			...
			App.mg.fillFloatingHandBackground_painttime = (gi.capEnv.fillFloatingHandBackground); // WIDE AREA FLAG - ugg
			App.mg.overlapShrink_painttime = (gi.capEnv.overlapShrink); // WIDE AREA FLAG - ugg
...
//			if (t == q_.nt) { consume_nt(gi); App.gbp.c2_0__btp.AlertDisplaySet(false); continue; }
			...
			if (t == q_.ZS) { consume_suitSymbol(gi, 's' /* extract the suit symbol */); continue; } 
			if (t == q_.Zd) { consume_emDash(gi); continue; } 
			if (t == q_.Zz) { consume_suitSymbol(gi, 'z' /* the home symbol */); continue; } 
			if (t == q_.Zr) { consume_suitSymbol(gi, '}' /* the right arrow symbol */); continue; } 
			if (t == q_.Zl) { consume_suitSymbol(gi, '{' /* the left arrow symbol */); continue; } 
			if (t == q_.Zm) { consume_suitSymbol(gi, 'm' /* checkmark */); continue; } 
			...
			if (t == q_.Z4) { consume_wipePartOfScreen(gi, 4 /* full_width */); continue; } 
			if (t == q_.Z5) { consume_wipePartOfScreen(gi, 5 /* restriced_width */); continue; } 
			if (t == q_.Z6) { consume_wipePartOfScreen(gi, 6 /* restriced ?? */); continue; } 
			if (t == q_.Z7) { consume_wipePartOfScreen(gi, 7 /* restriced ?? */); continue; } 
			if (t == q_.Z8) { consume_wipePartOfScreen(gi, 8 /* restriced ?? */); continue; } 
			if (t == q_.Z9) { consume_wipePartOfScreen(gi, 9 /* restriced ?? */); continue; } 
			...
			if (t == q_.jp) { /* nothing at this time */ continue; } 
			...
			if (t == q_.zd) { /* nothing at this time */ continue; } 
			if (t == q_.zg) { /* nothing at this time */ continue; } 
			if (t == q_.mx) { /* nothing at this time */ continue; } 
			if (t == q_.xl) { /* nothing at this time */ continue; } 
//			if (t == q_.bh) { /* nothing at this time */ continue; } 
//			if (t == q_.bi) { /* nothing at this time */ continue; } 
...
//			if (t == q_.su) { /* nothing at this time */ continue; } // su Special Use clear checkboxes
							
			// @formatter:on

			if (i == mg.end_pg) { // so this is the final one of the "display set"
				if (App.su_clear_overide == false) {
					if (gi.capEnv.zm_clear_su > 0) { /* 3 = always,  2 (and 1) = once */
						if (App.frame.rop.p2_KibSeat.is_compass_or_showAlwaysHidden_Set()) {
							App.frame.rop.p2_KibSeat.clear_compass_and_showAlwaysHidden();
						}
					}
				}
			}
		...
		if (ih_seen > 0 && App.mg.overlapShrink_painttime)
