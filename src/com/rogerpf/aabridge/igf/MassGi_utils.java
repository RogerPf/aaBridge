...
//		App.youSeatHint = App.deal.youSeatHint;
	...
	static public String getDealAsLinSave_BBOorSTD(Deal deal, String kib_seat, int new_number, boolean first_of_set, boolean single_deal_save) {
		...
		out += deal.PlayerNamesForLinFile(extra_space) + eol_or_blank;

		// Headers and our invented Display Board Number
		out += "rh||";

		String ahText = signfBoardId + " " + local_displayId;
		if (App.linfileSaveFormat == App.linFmt_Std) {
			if (deal.ahHeader.trim().isEmpty() == false) {
				ahText += "  " + Aaa.cleanString(deal.ahHeader, true /* true => spaceOk */).trim();
			}
		}
		...
		out += "" + (char) (((deal.dealer.v + 2) % 4 + 1 + '0'));
		...
		if (deal.endedWithClaim) {
			out += "mc|" + deal.tricksClaimed + "|" + pg_bars + eol_or_blank;
		}

		out += Zzz.get_lin_EOL() + Zzz.get_lin_EOL();
	...
	static public String getDealAsLinSave_Problem(Deal deal, String kib_seat, int new_number) {
		...
		String extra_EOL = Zzz.get_lin_EOL();
		...
		out += "md|"; // test A BRIDGE TABLE is displayed md => make deal ?
		...
		out += deal.cardsForLinSave(omitOpps) + "|" + EOL;
		...
		out += "at|    ^*bDo your thinking NOW^*n|pg||" + EOL + extra_EOL;

		out += "bv|f,v|" + EOL;
		out += "n#|h|cp|red|at|^^ ^d ^*bNow |cp||" + EOL;
		out += "at| Click^*n  the blue  { ^*b 1st ^*n }  button below|" + EOL;
		out += "cp|red|at|^*b  and play out the hand^*n|cp||" + EOL;
		out += "at|.     ^*b Flow ^*n  will end the problem.|" + EOL;
		out += "pf|first-button|" + EOL;
		out += "pg||" + EOL + extra_EOL;
		...
		if (s3.isEmpty() == false) {
			out += "bv|c,v|" + EOL;
			out += "ht|f|at|@4^^|" + EOL;
			out += "ht|m|at|^d Additional play follows ...|" + EOL;
			out += "sk||pf|both-first-cont|" + EOL;
			out += "pg||" + EOL + extra_EOL;
			out += s3;
		}
		...
		out += "ht|f|at|^e@4@5^^|" + EOL;
		out += "ht|k|at|^c The problem has finished.    If you move |" + EOL;
		out += "at|forward you will be shown all the cards.|" + EOL;
		if (omitOpps) {
			out += "at|^^^^|" + EOL;
		}
		else {
			out += "at|^^^^^c 'Study Deal'  restrictions lifted, |" + EOL;
			out += "at|hence in the deal the  ^*b DDS  { Off/On ^*n }  buttons, |" + EOL;
			out += "at|etc are |cp|red|at|^*b NOW ^*n|cp||at| available.|" + EOL;
		}
		out += "at|^^^^^L You can still use the blue  { ^*b 1st ^*n }  button.|" + EOL;
		out += "pg||" + EOL;
		out += "" + EOL;
		...
		out += "lg|c|" + EOL;
		out += "ht|f|at|@4|" + EOL;
		out += "ht|e|n^|5|at|^c@2^z@3^e  " + EOL;
		out += "|n#|3|at|" + EOL;
		out += "|" + EOL;
		out += "ht|w|at|^b^*bEnd^*n|" + EOL;
		out += "pf|all-res-lifted|" + EOL;
		out += "pg||" + EOL;
		out += "" + EOL;
	...
	public static String getDealAsLinSave(Deal deal_in, String kibSeat, int new_number, boolean first_of_set, boolean single) {
		...
		if (App.linSaveRotateDec && deal.contractCompass != Dir.South && deal.contractCompass != Dir.Invalid) {
...
class Hyperlink_h_ext extends Hyperlink {
	...
	private boolean checkForAndDoInternalHondo(String origUrl) {
		...
		if (App.hondo_use_local_copy == false)
			return false;
