...
public class Seg extends ArrayList<Ras> {
	...
	float rightMargin;
	Color fillColor;
	int wipePartOfScreen = 0; // 0 => dont   // 1 = full width   // 2 = restricted width
...
//				System.out.println(s); // <<<<<<<<<<<<<<<<<<<<<<<s
		...
		float w = 9000; // 9000 is any huge value
		float h = 9000; // 9000 is any huge value

		// wipePartOfScreen == 4  wipe down 
		// wipePartOfScreen == 5  wipe whole right size 
		// wipePartOfScreen == 6  wipe bottom right corner 
		// wipePartOfScreen == 7  wipe bottom top corner 

		if (wipePartOfScreen == 7) {
			x = get(0).x;
			h = get(0).y;
		}
		else if (wipePartOfScreen == 8) {
			w = get(0).x;
			y = get(0).y;
		}
		else if (wipePartOfScreen == 9) {
			w = get(0).x;
			h = get(0).y;
		}
		else {
			x = (wipePartOfScreen >= 5) ? get(0).x : 0.0f;
			y = (wipePartOfScreen != 5) ? get(0).y : 0.0f;
		}
