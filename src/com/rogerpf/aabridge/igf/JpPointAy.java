...
class JpPointAy extends ArrayList<JpPoint> {
	...
	public JpPointAy(MassGi mg) {
				...
				if (t == q_.qx) {
					...
					if (gi.bb.size() > 1) {
						...
						if (s.startsWith("hide")) {
							if (gi.bb.get(0).isEmpty())
								gi.bb.set(0, "-"); // empty will show as
		...
		/** 
		 * We now need to parse all the Jp's and calculate their locations on the line
		 */
		...
		/** the small have to share 5% of the total for their line thickness and 10% for their gap
