	...
	static Card act(Gather g) {
		...
		for (StraStep step : stra) {
			...
			if (step.idea == Strategy.RunSuit) {
				// ----------------------------------------------------
				Suit su = step.suit;
				int myHolding = h.frags[su.v].size();
				if (myHolding == 0)
					continue;
				int pnHolding = h.partner().frags[su.v].size();
				if (myHolding <= pnHolding) {
					card = h.frags[su.v].get(0);
				}
				else { // partners suit is shorter
					boolean topContig = h.areOurTopHoldingsContigious(su);
					if (topContig) {
						card = h.frags[su.v].getLast();
					}
					else {
						card = h.frags[su.v].get(0);
					}
			...
			if (step.idea == Strategy.RunTopTricksInSuit) {
				...
				if (myHolding == 0)
					continue;
				int pnHolding = h.partner().frags[su.v].size();
				if ((h.frags[su.v].get(0).rankEqu != Rank.Ace) && (pnHolding > 0) && (g.partner.frags[su.v].get(0).rankEqu != Rank.Ace)) {
					continue;
				}
				if (myHolding <= pnHolding) {
					card = h.frags[su.v].get(0);
