...
//		Hand h = g.hand;
		...
		int pnFragSize = g.partner.frags[card.suit].size();
		Card pnHighCard = null;
		if (pnFragSize > 0) {
			pnHighCard = g.partner.frags[card.suit].get(0);
		}
		...
		if (oppsRunDepth == 0) { // so we have the master
			...
			if (high.rankRel == Zzz.Ace) {
				// CHEAT WARNING
				boolean lhoHigh = g.LHO.frags[card.suit].size() > 0 && g.LHO.frags[card.suit].get(0).rankRel == Zzz.King;
				boolean rhoHigh = g.RHO.frags[card.suit].size() > 0 && g.RHO.frags[card.suit].get(0).rankRel == Zzz.King;
		...
		int ourInner = Hand.ourInnerRunDepth(g, suit, g.oppsBoth[suit].contigCards(0));

		if (Zzz.Ace - oppsRunDepth - ourInner > high.rankRel) { // TODO: THIS TEST NEED MORE WORK - is it broken? = what does it do
			return 1.0f; // no good our highest is not high enough
		}

		// can partner stop the opps taking more
		if (pnFragSize > oppsRunDepth && pnHighCard.rankEqu >= high.rankEqu) {
...
//			return card;
		...
		if (stra.signalWanted[suit] == false)
			return card;
		...
		if (card2.rank >= Zzz.Nine) // we don't signal with high cards
			return card;
...
//		boolean signalWanted = (fa.myFragLen % 2 == 1) ^ (defenderSignals == Zzz.StdEvenCount);
		...
		if (signalWanted) {
			...
			for (; i >= 0; i--) {
				Card c = frag.get(i);
				if (i < 2 && c.rank == card2.rank + 1) { // fine only if they are contigious
					card2 = c;
					continue;
				}
				if (c.rank >= Zzz.Nine)
