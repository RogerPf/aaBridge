		...
		if (g.haveSuitLed) {
			...
			if (g.bestByTrumping) { // nothing we can do
				...
				if (g.bestCard.rank.v > g.highestOfLed.rank.v) {
					...
					if (g.LHO.frags[g.suitLed.v].getLowestThatBeats(g.z, g.bestCard.rank) == null) {
						 ...
						 * And Can I do better ?
						 */
						Card LHO_plt = g.LHO.frags[g.suitLed.v].getHighestThatLosesTo(g.bestCard.rank);
						Card LHO_mlt = g.LHO.frags[g.suitLed.v].getHighestThatLosesTo(g.fragLed.get(0).rank);
				...
				// the LHO has this suit
					...
					Card LHO_posib_beater = g.LHO.frags[g.suitLed.v].getLowestThatBeats(g.z, posib.rank);
					if (LHO_posib_beater == null) {
						...
						else if (LHO_mlt.rank.v < posib.rank.v) {
							card = posib;
						}
						else {
							card = g.fragLed.getLowestThatBeats(g.z, LHO_mlt.rank);
