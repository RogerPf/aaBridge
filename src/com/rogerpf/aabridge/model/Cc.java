...
public class Cc {  // Color Class
	...
	public static final Color Yellowish          = new Color(223, 217, 49);
	...
	public static final Color GreenWeedy         = new Color(130, 180, 130);
...
//        clubs diamonds hearts  spades  no_trumps
	...
	static class C {
		...
		public void recalc(int index) {  // index is 0 = Tutorial    1 = Deal
			...
			int intensity = App.colorIntensity + notch + offset;			
			if (intensity < -255) {
				intensity = -255;
			}
			...
			else if (0 > intensity && intensity >= -255) {
				for (int i : Zzz.zto2) {
					wk.rgb[i] = dark.rgb[i] + ((med.rgb[i] - dark.rgb[i]) * (255 + intensity))/255;
				}
			}
			else if (0 <= intensity && intensity <= 255) {
				intensity = (int)Math.sqrt((double)intensity);
				for (int i : Zzz.zto2) {
