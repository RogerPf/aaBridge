...
public class Deal {
	...
	public boolean showXesValidInsideDeal = true;
	public boolean showXesValidPlayedCards = true;
	public int preset_trick_count_x = 0;
	public int preset_trick_count_y = 0;

	public int getPresetTrickCount() {
		return preset_trick_count_x + preset_trick_count_y;
	}

	public int eb_min_card = 0;
	public boolean suppress_autoshow_opening_lead = false;
	...
	public boolean zm_studyDeal_forced = false;

	public BotExtra botExtra = new BotExtra(this);

	public String zd_dealer_script = "";
	public String zg_merge_list = "";
	 ...
	 */
	// ==============================================================================================
	public boolean has52Cards() { // Have to have 52 cards
		int tot = 0;
...
//	}
	...
	public Deal() { /* Constructor */
		// used in emergency only when loading bad lins
		// ==============================================================================================
		this.localId = 0;
		localLast_pg = 0;
	}
	...
	/**
	 */
	public Deal(int localId /* ignored */) { /* Constructor */
		// used by Make_giAY
		// ==============================================================================================
	...
	public void deal_common(String dealType, Dir youSeatHint) { /* Constructor */
		...
		for (Suit suit : Suit.cdhs) {
			for (Rank rank : Rank.allThriteenRanks) {
				packPristine.add(new Card(suit, rank));
			}
		}
		...
		// the dumbauto player can remain unused and so easier to test using the 'tests'.
...
//		}
	...
	/**
	 */
	public boolean swapCards(Card swap, Card swapFor) {
		// ==============================================================================================

		Hand handAy[] = new Hand[1];

		Card card_A = getCardSearchAllHands_Frag(swap.suit, swap.rank, handAy);

		if (card_A == null) {
			return false; // we will not move a played card
		}
		Hand hand_A = handAy[0];

		Card card_B = getCardSearchAllHands_Frag(swapFor.suit, swapFor.rank, handAy);
		if (card_B == null) {
			return false; // we will not move a played card
		}
		Hand hand_B = handAy[0];

		(hand_A.fOrgs[card_A.suit.v]).remove(card_A);
		(hand_A.frags[card_A.suit.v]).remove(card_A);

		(hand_B.fOrgs[card_B.suit.v]).remove(card_B);
		(hand_B.frags[card_B.suit.v]).remove(card_B);

		hand_A.addDeltCard(card_B);
		hand_B.addDeltCard(card_A);
	...
	/**
	 */
	public Card getCardSearchAllHands(Suit suit, Rank rank) {
		// ==============================================================================================
		for (Hand hand : hands) {
	...
	/**
	 */
	public Card getCardSearchAllHands_Orig(Suit suit, Rank rank) {
		// ==============================================================================================
		for (Hand hand : hands) {
	...
	/**
	 */
	public Card getCardSearchAllHands_Frag(Suit suit, Rank rank, Hand[] handAy) {
		// ==============================================================================================
		for (Hand hand : hands) {
			Card card = hand.getCardIfMatching(suit, rank);
			if (card != null) {
...
//	}
	...
	/**
	 */
	public void fillDealExternal(ArrayList<String> hsAy, String action, Lin lin, int line_no_info, int page_numb_display) {
		// ==============================================================================================

		boolean insertMode = false;
//		boolean dealerSet = false;

		String bbinf = ((lin != null) ? lin.filenaDisp : "") + "Line " + line_no_info + "  pg " + page_numb_display + "   ";
		...
		if (maxSeats > 0) {
			...
			if ((hs.length() > 0) && (Character.toLowerCase(hs.charAt(0)) == 'p')) {
				hsAy.set(0, hs.substring(1)); // we dont support pictures of cards
			}
		}
...
//			   System.out.println(bbinf + "suit_symb == false  "  +  " >" + hsAy.get(0) + "< ");
...
//					}
					...
					Card card = packPristine.getIfSuitAndRankExists(suit, rank);
					if (card != null) {
						 ...
						 */
						card = getCardSearchAllHands(suit, rank);
						if (card == null) {
							System.out.println(bbinf + "md  -  Card not in deck " + rank + " " + suit);
							continue;
						}

						boolean success = removeCardIncFOrgs(card);
						if (success == false) {
							System.out.println(bbinf + "md  -  Can't remove played card " + rank + " " + suit);
							continue;
						}
					}
					/** 
					...
					// System.out.println(bbinf + hand + " " + card + "  ");
				}
			}
		}
		else {
...
//						//
//                      // leaving this code active break   md|0...
//                      // 
//						card = getCardSearchAllHands_Orig(suit, rank);
//						if (card != null) {
//							continue; // lets hope this is an auto added card and we are doing an md|0...
//						}

						card = getCardSearchAllHands(suit, rank);
						if (card == null) {
							System.out.println(bbinf + "b Card not in deck " + rank + " " + suit);
							continue;
						}

						boolean success = removeCardIncFOrgs(card);
						if (success == false) {
							System.out.println(bbinf + "Can't remove played card " + rank + " " + suit);
							continue;
						}
						if (insertMode == false) {
							System.out.println(bbinf + "duplicated card>>> " + card);
						}
					}
					/** 
					 * Now that the card has been removed from the pack or from a hand
...
//		}
	...
	public void markCardsKept(ArrayList<String> hsAy) {
		...
		for (int j = 0; j < hs.length(); j++) {
			 ...
			 */
			card = getCardSearchAllHands_Orig(suit, rank); // ORIG ORIG ORIG ORIG <<<<<<<<<<<<<<<<<
			if (card == null) {
				System.out.println("MarkCardKept kc  -  Card not in deck " + rank + " " + suit);
				continue;
			}
	...
	public void removeCards(ArrayList<String> hsAy) {
		...
		if (hs.toLowerCase().contentEquals("all")) {
			hs = "sAKQJT98765432 hAKQJT98765432 dAKQJT98765432 cAKQJT98765432";  //  remove all
		}
	...
	public Deal deepClone() {
		...
		d.showXesValidInsideDeal = showXesValidInsideDeal;
		d.showXesValidPlayedCards = showXesValidPlayedCards;
		d.eb_min_card = eb_min_card;
		d.suppress_autoshow_opening_lead = suppress_autoshow_opening_lead;
		d.preset_trick_count_x = preset_trick_count_x;
		d.preset_trick_count_y = preset_trick_count_y;

		d.dfcDeal = dfcDeal;
		d.forceDifferent = forceDifferent;

		d.zm_studyDeal_forced = zm_studyDeal_forced;

		d.botExtra = new BotExtra(d, botExtra);

		d.zd_dealer_script = zd_dealer_script;
		d.zg_merge_list = zg_merge_list;
		...
		for (Dir p : Dir.nesw) {
			...
			for (Suit su : Suit.cdhs) {
				...
				d_fOrg.showPlayedVis = o_fOrg.showPlayedVis;
				// hand, suit, suitCh all preset
				for (Card o_card : o_fOrg) {
					Card d_card = d.packPristine.getIfSuitAndRankExists(o_card.suit, o_card.rank);
//					if (d_card == null) {
//						@SuppressWarnings("unused")
//						int z = 0;
...
//					}
			...
			for (Suit su : Suit.cdhs) {
				...
				d_frag.showPlayedVis = o_frag.showPlayedVis;
				// hand, suit, suitCh all preset
...
//			}
	...
	/***
	 *   SUsed to set x'es etc for floating hands
	 */
	public void setShowXesEtc(Deal from) {

		for (Dir p : Dir.nesw) {
			Hand o_hand = from.hands[p.v];
			Hand d_hand = this.hands[p.v];
	...
	/**
	 */
	public int coreCompare(Deal d) {
		/* 0 => Different (not Equal)
		 * 1 => Similar but WE (this) have missing orig cards NOT GENERATED at the moment
		 * 2 => SAME
		 * 3 => Similar but WE (this) have extra orig cards
		 */
		// ==============================================================================================
		if (qx_room != d.qx_room)
			return 0; // this is the Open an Closed state of the room
		...
		for (Dir p : Dir.nesw) {
			...
			for (Suit su : Suit.cdhs) {
				...
				else if (o_fOrg.size() > d_fOrg.size()) {
					we_have_extra = true;
					for (int i = 0; i < d_fOrg.size() - 1; i++) {
						Card d_card = d_fOrg.get(i);
						boolean found = false;
						for (int j = 0; j < o_fOrg.size() - 1; j++) {
							Card o_card = o_fOrg.get(j);
							if (o_card.rank == d_card.rank) {
								found = true;
								continue;
							}
		...
		return we_have_extra ? 3 : 2; // 2 = equals 
	}
...
//				int z = 0;
			...
			if (checkForInteresting) {
				...
				targetTricks = d.contract.level.v + 6;

				ddsTricks = Z_ddsCalculate.calcMaxDeclarerTricks(d);
				...
				dumbTricks = d.getContractTrickCountSoFar().x;
...
//						      + "    dds: " + ddsTricks + " " + dds
//						      + "    md|" + d.cardsForLinSave(false /* all hands */) + "|");
//				// @formatter:on
		...
		bestSoFar.wipeContractBiddingAndPlay();

		if (presetTheContract && (((youSeatForNewDeal.v % 2) == Dir.EW) || (southBiddingRequired(criteria) == false))) {
			bestSoFar.autoBidContract(criteria);
		}

//		System.out.println("df:" + dealFilter + "  i: " + i + "    Board        md|" + bestSoFar.cardsForLinSave(false /* all hands */) + "|  ");
...
//	public void rotateAllDealTo(Dir compass) {           xxxxxxxxxxxxxx
//		// ==============================================================================================
//		int rot =  (8 - compass.v) % 4;
//		if (rot == 0)
//			return;
//		rotateDealerAndVulnerability( rot);
//	}
//
	/**
	 */
...
//	}
	...
	public void undoLastPlays_ignoreTooMany(int numb) {
		// ==============================================================================================
		int played = countCardsPlayed();
		for (int zz = 0; zz < numb; zz++) {
			Hand hand = getLastHandThatPlayed();
			if (hand == null)
				break; // too many
			if (--played <= eb_min_card) {
				eb_min_card = played;
			}
			hand.undoLastPlay();
		}
...
//		}
	...
	*/
	...
	public String cardPlayForLinSave(int stage /* stage 0, 1-3 */) {
		String s = "";

		String eol_or_blank = (App.linfileSaveFormat == App.linFmt_BBO) ? "" : Zzz.get_lin_EOL();
		String extra_space = (App.linfileSaveFormat == App.linFmt_BBO) ? (App.bboUpStripped ? "" : " ") : "";

		String eb_y = "eb|y___|";

		if ((App.linfileSaveFormat == App.linFmt_BBO) && App.bboUpStripped) {
			eb_y = "";
		}
		...
		if (stage == 0) {
			...
			/* 
			 * The original 
			 * nothing to do with 'problem' save style 
			 * */
			...
			for (int i = 0; i < countPlayed; i++) {
				...
				if ((eb_min_card > 1 && eb_min_card == i + 1) /* || (i == 0 && eb_min_card == 0) */) {
					s += eb_y;
				}
				if (i == 0 && countPlayed > 1) {
					s += "pg|" + extra_space + "|" + eol_or_blank; // add an 'extra' pg|| after the first card
				}
				tk = (tk + 1) % 4;
				if (tk == 0) {
					s += "pg|" + extra_space + "|" + eol_or_blank;
		...
		if (stage == 1) {
			if (countPlayed > 0) {
				Card card = getCardThatWasPlayed(0);
				s += "pc|" + card.suit.toCharLower() + "" + card.rank.toChar() + "|";
				if (eb_min_card <= 1) {
					s += eb_y;
				}
		...
		if (stage == 2) {
			...
			int upto = (eb_min_card > 0) && (eb_min_card < countPlayed) ? eb_min_card : countPlayed;

			for (int i = 1; i < upto; i++) {
				Card card = getCardThatWasPlayed(i);
				s += "pc|" + card.suit.toCharLower() + "" + card.rank.toChar() + "|";
				if (eb_min_card > 0 && eb_min_card == i + 1) {
					s += eb_y;
				}
				tk = (tk + 1) % 4;
				if (tk == 0) {
			...
			if (!s.isEmpty() && (tk != 0)) // we ended part way through a trick
				s += "pg|" + extra_space + "|" + eol_or_blank;
		...
		if (stage == 3) {
			...
			int from = (eb_min_card <= 1) ? 1 : eb_min_card;

			int tk = from % 4;

			for (int i = from; i < countPlayed; i++) {
				Card card = getCardThatWasPlayed(i);
				s += "pc|" + card.suit.toCharLower() + "" + card.rank.toChar() + "|";
				tk = (tk + 1) % 4;
				if (tk == 0) {
	...
	public void compressBiddingToMinimum() {
		...
		int p = (8 + contractCompass.v - dealer.v) % 4;
		for (int i = p; i > 0; i--) {
			getNextHandToBid().bids.add(new Bid(Call.Pass));
		}

		Bid b = new Bid(contract.level, contract.suit);
		getNextHandToBid().bids.add(b);
		getNextHandToBid().bids.add(new Bid(Call.Pass));
		getNextHandToBid().bids.add(new Bid(Call.Pass));
		getNextHandToBid().bids.add(new Bid(Call.Pass));
	...
	public void removeFirstBidFixingDealer() {
		Hand hand = hands[dealer.v];
		if (hand.bids.size() > 0) {
			hand.bids.remove(0);
			dealer = dealer.nextClockwise();
	...
	public void adjustDealerToSouthWithPasses() {

		if (dealer == Dir.South) {
			return;
		}
		if (isDeclarerValid() == false) {
			setDealer(Dir.South);
			return;
		}

		while (getFirstBid().getCall() == Call.Pass) {
			removeFirstBidIfPassFixingDealer();
		}

		while (dealer != Dir.South) {
			addPassAsFirstBidFixingDealer();
		}
	}

	public void removeFirstBidIfPassFixingDealer() {
		Hand hand = hands[dealer.v];

		if (hand.bids.size() > 0 && hand.bids.get(0).call == Call.Pass) {
			hand.bids.remove(0);
			dealer = dealer.nextClockwise();
		}
	}

	public void addPassAsFirstBidFixingDealer() {
		Dir new_dealer = dealer.prevAntiClockwise();
		Hand hand = hands[new_dealer.v];
		hand.bids.add(0, new Bid(Call.Pass));
		dealer = new_dealer;
	}
...
//		return ((s[0] == s[1]) && (s[1] == s[2]) && (s[2] == s[3]));
	...
	/**
	 */
	public boolean fewerThanTwoCardsPlayed() {
...
//		return ((s[0] == s[1]) && (s[1] == s[2]) && (s[2] == s[3] + 1));
	...
	public void claimTricks(int claimed) {
		// ==============================================================================================
		...
		if (claimed > 13 - score.y) { // they are claiming DEFENDERS WON tricks !!!
			claimed = 13 - score.y;
		}

		if (claimed < score.x) { // claiming less than they have already made !!!
			claimed = score.x;
		}

		endedWithClaim = true;
		tricksClaimed = claimed;
	}
	...
	public Point getContractTrickCountSoFar() {
		// ==============================================================================================
		Point trickCount = new Point(this.preset_trick_count_x, preset_trick_count_y);
		...
		if (trickCount.y > (13 - trickCount.x))
			trickCount.y = (13 - trickCount.x);
	 ...
	 * returns an int
	 */
	public int getContractTrickCountForDirecton_exclude_last_if_ours(Dir compass) {
		// ==============================================================================================
		int trickCount = 0;
		boolean skipFirst = true;
		...
		return trickCount - ((last_us == 1) && (countCardsPlayed() % 4 == 0) ? 1 : 0);
	}

	/**
	 * returns an int
	 */
	public int getContractTrickCountForDirecton_exclude_last(Dir compass) {
		// ==============================================================================================
		int trickCount = 0;
		boolean skipFirst = true;
		Dir partnerCompass = compass.rotate180();
	...
	/** 
	 * We are counting all the LED cards of the given suit.
	 */
	int countLedOfSuit(Suit suit) {
		int count = 0;

		int trick = -1;
		for (Hand leader : prevTrickWinner) {
			trick++;
			if (leader.played.size() <= trick)
				break;
			if (leader.played.get(trick).suit.v == suit.v) {
				count++;
			}
		}
		return count;
	...
	boolean isActiveCard(Frag fragSkip, Rank rank, Suit suit) {
		...
		for (Hand hand : hands) {
			...
			if (hand.played.size() == prevTrickWinner.size() && hand.played.getLast().matches(suit, rank)) {
...
//			}
	...
	public void setAllrankRelEquMpat_part1(boolean trick_anal_time, Hand us, Mpat[] mpatAyEqu, Mpat[] mpatAyRel, int positionInTrick, int debug_trickNumb) {
		...
		// generate all the mpat and epat "Card Pattern matchers"
		if (trick_anal_time) {
...
//		}
	...
	public int completenessScore() {
		int score = countOriginalCards();
		if (score == 52) {
			score = 100;
		}
		if (isBidding()) {
			return score + 100 + 1 + countBids();
		}

//      // Original OK (good) BUT does not recognise any eb_min_card change	
//		if (isPlaying() || isFinished()) {
//			int countPlayed = countCardsPlayed();
//			return score + 200 + countPlayed                       + 60 * ((countPlayed > 0) && isFinished() ? 1 : 0);
//		}	
		if (isPlaying() || isFinished()) {
			int countPlayed = countCardsPlayed();
			return score + 200 + (countPlayed * 13) + eb_min_card + 60 * ((countPlayed > 0) && isFinished() ? 13 : 0);
		}
		return 0;
	}
	...
	public void swapOppsHands() {
		...
		if (goodGuys == Dir.NS) {
			Hand h = hands[Dir.East.v];
			hands[Dir.East.v] = hands[Dir.West.v];
			hands[Dir.West.v] = h;
		}
		else {
			Hand h = hands[Dir.North.v];
			hands[Dir.North.v] = hands[Dir.South.v];
			hands[Dir.South.v] = h;
		}
	...
	/** 
	 */
	public String cardsForLinSave(boolean omitDefenders) {
		String s = "";
		for (Hand hand : rota[Dir.South.v]) {
			if (omitDefenders && (contractCompass != Dir.Invalid) && hand.axis() != contractCompass.axis()) {
				; // do nothing
			}
			else {
	...
	public void setDealShowXes(ArrayList<String> as) {
		...
		if (as == null) {
			showXesValidInsideDeal = true;
			showXesValidPlayedCards = true;
	...
	public void setDealShowPlayedVis(ArrayList<String> as) {
	...
	static String encapsulate(String text, String marker) {
		text = text.trim();
		if (text.isEmpty())
			return text;
		else
	...
	static String hyphenIfEmpty(String text) {
		return text.isEmpty() ? "-" : text;
	}

	public void writePbnToSaveableArray(ArrayList<String> ay, int new_number, boolean first_of_set) {

		String info = encapsulate(zd_dealer_script, Aaa.zd_dealer_script_marker);
		if (info.isEmpty())
			info = "-";
		ay.add("[Event \"" + info + "\"]");
		...
		info = "";
		if (youSeatHint != Dir.Invalid)
			info = encapsulate("" + youSeatHint.toChar(), Aaa.sk_seat_kib_marker);
		if (first_of_set)
			info += encapsulate(zg_merge_list, Aaa.zg_merge_list_marker);
		if (info.isEmpty())
			info = "-";
		ay.add("[Site \"" + info + "\"]");
		...
		ay.add("[Date \"-\"]");  // we do not preserve the 'pbn' date

		ay.add("[Board \"" + ((new_number > 0) ? new_number : board_id) + "\"]");

		ay.add("[West \"" + hyphenIfEmpty(hands[Dir.West.v].playerName) + "\"]");
		ay.add("[North \"" + hyphenIfEmpty(hands[Dir.North.v].playerName) + "\"]");
		ay.add("[East \"" + hyphenIfEmpty(hands[Dir.East.v].playerName) + "\"]");
		ay.add("[South \"" + hyphenIfEmpty(hands[Dir.South.v].playerName) + "\"]");
	...
	public String getSingleVulnerabilityLetter() {

		if (vulnerability[Dir.NS] && vulnerability[Dir.EW])
			return "b";
		else if (vulnerability[Dir.NS])
			return "n";
		else if (vulnerability[Dir.EW])
			return "e";

		return "";
	}

	static public String getSingleVulnerabilityLetterFromBoardNumber(int numb) {

		boolean vulnerability[] = Zzz.getBoardData(numb).vulnerability;
	...
	int countOriginal_declarer_cards() {
		if (contractCompass == Dir.Invalid)
			return 0;
		else if (contractCompass == Dir.North || contractCompass == Dir.South)
			return hands[Dir.North.v].countOriginalCards() + hands[Dir.South.v].countOriginalCards();
		else
			return hands[Dir.East.v].countOriginalCards() + hands[Dir.West.v].countOriginalCards();
	}

	public int countOriginal_defender_cards() {
		if (contractCompass == Dir.Invalid)
			return 0;
		else if (contractCompass == Dir.East || contractCompass == Dir.West)
			return hands[Dir.North.v].countOriginalCards() + hands[Dir.South.v].countOriginalCards();
		else
			return hands[Dir.East.v].countOriginalCards() + hands[Dir.West.v].countOriginalCards();
	}

	public String PlayerNamesForLinFile(String extra_space) {
		String out = "";
		// @formatter:off
		if ((   hands[Dir.South.v].playerName.isEmpty() 
			 && hands[Dir.West.v ].playerName.isEmpty() 
			 && hands[Dir.North.v].playerName.isEmpty() 
			 && hands[Dir.East.v ].playerName.isEmpty()
			) == false) {			
			out += "pn|";
			for (Hand hand : rota[Dir.South.v]) {
				out += hand.playerName;
	...
	public Dir lho_dir() {
		if (contractCompass == Dir.Invalid)
			return Dir.Invalid;
		return contractCompass.nextClockwise();
	}

	public Dir rho_dir() {
		if (contractCompass == Dir.Invalid)
			return Dir.Invalid;
		return contractCompass.prevAntiClockwise();
	}
