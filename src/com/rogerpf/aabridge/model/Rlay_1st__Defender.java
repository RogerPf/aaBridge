...
//		}
		...
		Suit rtnPartnersSuit =     
				 g.ptnrIsOpeningLeader 
             && (g.suitOfOpeningLeader != null) 
             && (h.frags[g.suitOfOpeningLeader.v].size() > 0)
             && (g.partner.frags[g.suitOfOpeningLeader.v].size() > 0)
             && (g.partner.frags[g.suitOfOpeningLeader.v].size() > h.frags[g.suitOfOpeningLeader.v].size())
             && (g.timesWeHaveLed == 1 /* this is it our first */)
           ?     g.suitOfOpeningLeader   :   null;
		
		if (rtnPartnersSuit != null) {
			// Frag frag = g.hand.frags[g.suitOfOpeningLeader.v];
			for (FragAnal fa : g.fragAnals) {
				if (fa.myFrag.suit.v == g.suitOfOpeningLeader.v) {
					card = pick_high_or_low(fa, h);
					if (card != null) {
						// final check that partner has a higher or equal card than ours
						if (card.rankEqu.v <= g.partner.frags[g.suitOfOpeningLeader.v].getFirst().rankEqu.v) {
							return card;
						}	
					}
				}
			}
			card = null; // then maybe not
		}
		...
		for (int fa_i = 0; fa_i < fa_len; fa_i++) {
			...
			card = pick_high_or_low(fa, h);
			Card second = fa.myFrag.getSecond();
			Suit suit = card.suit;
			...
			if ((fa_i == fa_len - 1) || ((g.fragAnals[fa_i + 1]).myFragLen == 0)) {
				if (card.rankRel == Rank.King 
				     && (second.rankRel.v < Rank.Queen.v) 
				   ) {
					card = second;
				}
