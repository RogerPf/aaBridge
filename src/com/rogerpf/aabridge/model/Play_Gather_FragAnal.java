...
class Gather {
	...
	boolean ptnrIsOpeningLeader ;
	Suit suitOfOpeningLeader    ;
	...
	Rank     pnHighestOfLedRank;
	
	int      timesWeHaveLed;
	
	Suit     debug_suit = Suit.Invalid;
	
	int      finMaDepth = 9; // used by Finesse patternMatching to pass down the finesse depth param, 9 means there will be no limit
	...
	Gather(Hand h, DumbAutoDirectives dumbAutoDir, boolean strategyCreated) { // Constructor and everything
		...
		positionInTrick		= (4 + h.compass.v - leader.compass.v) % 4;  // 0 = leader
		trickNumb 			= deal.getCurTrickIndex();
		compass 			= h.compass;
		compassV			= h.compass.v;
		...
		declarer			= deal.hands[h.deal.contractCompass.v];
		declarerCompass		= h.deal.contractCompass;
		declarerAxis		= h.deal.contractAxis();
		leaderUs 			= (h == leader);
		posFromDeclarer		= (4 + h.compass.v - declarerCompass.v) % 4;
		
		declarersSide		= posFromDeclarer % 2 == 0;
		defendersSide		= posFromDeclarer % 2 == 1;

		cardLed 			= (leaderUs) ? null : leader.played.getLast();
		rankLed 			= (cardLed == null) ? Rank.Invalid : cardLed.rank;
		suitLed 			= (cardLed == null) ? Suit.Invalid : cardLed.suit;
		trumpSuit 			= deal.contract.suit;
		noTrumps 			= (trumpSuit == Suit.NoTrumps);
		trumpContract       = !noTrumps;
		areWeWinning 		= (cardLed == null) ? false : h.areWeWinning();
		ourContract 		= (deal.contractAxis()) == h.axis();
		z					= ourContract;
		myTrumps 			= (noTrumps) ? 0 : h.frags[trumpSuit.v].size();
		pnTrumps 			= (noTrumps) ? 0 : h.partner().frags[trumpSuit.v].size();
		haveSuitLed 		= (cardLed == null) ? false : (h.frags[suitLed.v].size() > 0);
		pnHaveSuitLed 		= (cardLed == null) ? false : (h.partner().frags[suitLed.v].size() > 0);
		...
		lowestTrump 		= (noTrumps) ? null : h.frags[trumpSuit.v].getLast();
		lowestOfLed			= (leaderUs) ? null : h.frags[suitLed.v].getLast();
		highContLowestOfLed = (leaderUs) ? null : h.frags[suitLed.v].highestContigWith(lowestOfLed);
		highestTrump 		= (noTrumps) ? null : h.frags[trumpSuit.v].getCard(0);
		highestOfLed		= (leaderUs) ? null : h.frags[suitLed.v].getCard(0);
		
		tricksPlayed		= trickNumb;
		tricksRemaining     = 13 - tricksPlayed;
		Point trickCountPt  = h.deal.getContractTrickCountSoFar();
		declarersTarget     = 6 + h.deal.contract.level.v - trickCountPt.x;
		...
		openingLeader       = deal.hands[(declarer.compass.v + 1) % 4];
		
		ptnrIsOpeningLeader = (openingLeader.compass.v == partner.compass.v);		
		suitOfOpeningLeader = (openingLeader.played.size() == 0) ? null : openingLeader.played.get(0).suit;
		
		timesWeHaveLed      = deal.prevTrickWinner.timesLed(hand.compass);
		
		
		if (suitOfOpeningLeader == trumpSuit) {
			suitOfOpeningLeader = null;
		}
		...
		for (Suit su : Suit.cdhs) {
			ourBoth[su.v]        = (Frag)(h.frags[su.v].clone());
			oppsBoth[su.v]       = (Frag)(LHO.frags[su.v].clone());
		}	
		h.addPartnersCurrentCards( ourBoth);
		LHO.addPartnersCurrentCards( oppsBoth);
		
		for (Suit su : Suit.cdhs) {
			fragAnals[su.v] = new FragAnal(h.frags[su.v]);
			FragAnal fa = fragAnals[su.v];

			fa.myFrag           = h.frags[su.v];
			fa.pnFrag           = partner.frags[su.v];
			fa.frag[0]          = fa.myFrag;
			fa.frag[1]          = fa.pnFrag;
			...
			Frag myOrigFrag     = h.fOrgs[su.v];
			Frag pnOrigFrag     = partner.fOrgs[su.v];
			...
			fa.ourTopContigCards = ourBoth[su.v].contigCards(0);
			fa.ourTopTricksRaw  = Math.min(fa.ourTopContigCards, fa.ourMaxFragLen);
			fa.ourTopTricksCor  = fa.ourTopTricksRaw;
			fa.ourMissingKing   = ourBoth[su.v].isMissingKing();
			fa.ourMissingAceQueen = ourBoth[su.v].isMissingAceQueen();
			
			// the two opps
			fa.oppsLhoFragLen   = LHO.frags[su.v].size();
			fa.oppsRhoFragLen   = RHO.frags[su.v].size();
			...
			fa.oppsTopContigCards = oppsBoth[su.v].contigCards(0);
			...
			fa.ourInnerRun    = ourBoth[su.v].contigCards(fa.oppsTopContigCards);
			fa.oppsInnerRun   = oppsBoth[su.v].contigCards(fa.ourTopContigCards);

			if (fa.myFragLen > 0 && fa.pnFragLen > 0) {
				Card pnHigh = fa.pnFrag.get(0);
				fa.isEntryIntoPartner = (pnHigh.rankEqu == Rank.Ace) && (pnHigh.rank.v > fa.myFrag.getLast().rank.v);
			}
			
			fa.mpatEqu = mpatAyEqu[su.v];
			fa.mpatRel = mpatAyRel[su.v];
			...
			if (fa.ourTopTricksCor > 0 && fa.myFragLen > 0 && fa.pnFragLen > 0) {
				...
				for (Card card : fa.myFrag) {
					if (card.rankEqu != Rank.Ace) break;
					if (--pnl < 0) break;
					if (card.rank.v > fa.pnFrag.get(pnl).rank.v)
						my ++;
				}
				...
				for (Card card : fa.pnFrag) {
					if (card.rankEqu != Rank.Ace) break;
					if (--myl < 0) break;
					if (card.rank.v > fa.myFrag.get(myl).rank.v)
						pn ++;
				}
		...
		outstandingTrumps = (noTrumps) ? 0 : fragAnals[trumpSuit.v].outstanding;
		isTopOutstandingTrumpMaster = (noTrumps || oppsBoth[trumpSuit.v].isEmpty()) ? false : (oppsBoth[trumpSuit.v].get(0).rankRel == Rank.Ace);
		
		fragLed = (cardLed == null) ? null : h.frags[cardLed.suit.v];
		fragTrumps = (noTrumps) ? null : h.frags[trumpSuit.v];
		
		pnFragLed = (cardLed == null) ? null : partner.frags[cardLed.suit.v];
		pnFragTrumps = (noTrumps) ? null : h.frags[trumpSuit.v];

		pnHasSuitLed 		= (pnFragLed == null) ? false : (pnFragLed.size() > 0);
		pnHighestOfLedRank  = (pnHasSuitLed == false) ? Rank.Invalid : pnFragLed.get(0).rank;
		
		faLed = (cardLed == null) ? null : fragAnals[cardLed.suit.v];
		faTrumps = (noTrumps) ? null : fragAnals[trumpSuit.v];
