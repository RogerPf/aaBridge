...
public class Frag extends Cal {
	...
	public void setShowXes(char c) {
		...
		switch (c) {
		// @formatter:off
			case '*': // asterisk covers up / blots out / hides
			case 'x':
			case 't':
			case 'j':
			case 'q':
			case 'k':
			case 'a': showXes = c;	 break;
		    default:  showXes = '-'; break; // includes 'c' etc
		// @formatter:on
		}
	}

	public void setShowPlayedVis(char c) {
		if (c == '-')
			return; // no change
		...
		// @formatter:off
		    case 'd': 
		    case 'p':
			case '*': showPlayedVis = c;   break; // asterisk covers up / hides
			default:  showPlayedVis = '-'; break; // so 'c' etc clear
		// @formatter:on
		}
	}
	...
	public char getShowPlayedVis() {
		return showPlayedVis;
	}
