...
class Qlay_5_Discard {
	...
	public static Card pickBest(Gather g) {
		...
		for (int i : Zzz.rota[Zzz.Diamonds]) {
			Frag frag = h.frags[i];
...
//		Hand h = g.hand;
		...
		if (oppsRunDepth == 0) { // so we have the master
			...
			if (high.rankRel == Zzz.Ace) {
				// CHEAT WARNING
// @formatter:off
				if (   (!g.LHO.frags[card.suit].isEmpty() && g.LHO.frags[card.suit].get(0).rankRel == Zzz.King)
					|| (!g.RHO.frags[card.suit].isEmpty() && g.RHO.frags[card.suit].get(0).rankRel == Zzz.King)
					) {			
// @formatter:on
