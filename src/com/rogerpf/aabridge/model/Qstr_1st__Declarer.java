	...
	static void act(Strategy stra, Gather g) {
		...
		if (g.drawTrumpsHint) {
			...
			if (Play_Mpat.isPatternMatch(g, g.trumpSuit, Zzz.Leader_Pos, Zzz.MatchAsSelf)) {
				stra.add(new StraStep(Strategy.PlayCrd, "a finesse in trumps", g.mpatRtn.rankRel, g.trumpSuit, Dir.Invalid));
				return;
			}
			...
			if (tryToEnterPartner) {
				...
				if (card != null) {
					stra.add(new StraStep(Strategy.PlayCrd, "to get to partners hand", card.rankRel, card.suit, h.partner().compass));
					return;
				}
			}
			...
			// TO DO: ADD - can we find a lead that partner can trump ?
		}
		...
		if (g.ourTopTricksTot >= g.tricksRemaining || g.ourTopTricksTot >= g.ourTarget) {
			...
			stra.add(new StraStep(Strategy.RunTopTricksInSuit, "", Rank.Invalid, g.fragAnals[0].suit, Dir.Invalid));
			return;
		}
		...
		if (g.haveTrumps && g.pnHaveTrumps) {
			...
			if (g.myTrumps >= g.pnTrumps) {
				...
				if (fa.mpatRtn.rating > 0) {
					stra.add(new StraStep(Strategy.PlayCrd, "for Partner to Ruff", fa.myFrag.getLast().rankRel, fa.suit, Dir.Invalid));
				}
			}
			...
			if (g.myTrumps <= g.pnTrumps) { // pn has more trumps that I do (or equal and no Xruff earlier)
				...
				if (fa.mpatRtn.rating > 0) {
					Card card = Strategy.selectBestEntryIntoPartner(g);
					if (card != null) {
						// System.out.println(h.compass.toLongStr() + "  GetToPartner " + card);
						stra.add(new StraStep(Strategy.PlayCrd, "get to partner", card.rankRel, card.suit, h.partner().compass));
						return;
					}
				}
			}
		...
		// well do we have a good finesse to take ?
		{
			FragAnal fa = g.fragAnals[0];
			if (fa.mpatRtn.rating > 0) {
				Card card = h.frags[fa.suit.v].getIfRelExist(fa.mpatRtn.rankRel);
				if (card == null) {
					@SuppressWarnings("unused")
					int z = 0; // put your breakpoint here
				}
				assert (card != null);
				// System.out.println(h.compass.toLongStr() + "  TakeFinesse " + card);
				stra.add(new StraStep(Strategy.PlayCrd, "take finesse", fa.mpatRtn.rankRel, fa.suit, Dir.Invalid));
				return;
			}
		}
