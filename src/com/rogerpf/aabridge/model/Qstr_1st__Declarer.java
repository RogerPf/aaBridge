	...
	static void act(Strategy stra, Gather g) {
		...
		if (g.drawTrumpsHint) {
			...
			if (Play_Mpat.isPatternMatch(g, g.trumpSuit, Zzz.Leader_Pos, Zzz.MatchAsSelf)) {
				stra.add(new StraStep(Strategy.PlayCard, "a finesse in trumps", g.mpatRtn.rankRel, g.trumpSuit, Dir.Invalid));
				return;
			}
		...
		if (g.haveTrumps && g.pnHaveTrumps) {
			...
			if (g.myTrumps >= g.pnTrumps) {
				...
				if (fa.mpatRtn.rating > 0) {
					stra.add(new StraStep(Strategy.PlayCard, "for Partner to Ruff", fa.myFrag.getLast().rankRel, fa.suit, Dir.Invalid));
				}
			}
		...
		// well do we have a good finesse to take ?
		{
			...
			if (fa.mpatRtn.rating > 0) {
				...
				stra.add(new StraStep(Strategy.PlayCard, "take finesse", fa.mpatRtn.rankRel, fa.suit, Dir.Invalid));
				return;
			}
		}
