...
//		int playedThisTrick = 0;
	...
	public static Deal scoreCardsInNextHandToPlay(Deal abDeal) {
		...
		int countCardsPlayed = abDeal.countCardsPlayed();
		int tricksPlayed = countCardsPlayed / 4;
		int playedThisTrick = countCardsPlayed % 4;
			...
			for (ddsRank = 14; (equals != 0); ddsRank--) {
				...
				if ((equals & bitRank) != 0) {
					...
					if (abCard != null) { // how can it be null
						int ajScore = score + ((abHand.axis() == abDeal.contractAxis()) ? abDeal.preset_trick_count_x : abDeal.preset_trick_count_y);
						if (ajScore > 13)
							ajScore = 13;
						abCard.setDdsScore(ajScore);
					}
				}
			}
		}
		// System.out.println("ddsCalc ddsScore Trick no = " + playedThisTrick);

		App.ddsKeptHands[tricksPlayed][playedThisTrick] = abHand;

		return abDeal; // to be kept (if wanted) as the  ddsDeal
	}
