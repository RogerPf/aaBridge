...
class DePoint {
	...
	DePoint(int index, char type, char state, String text, float from, float to, boolean spike, int bidInd, int trickInd, int cardInd) {
		...
		this.spike = spike;
		this.bidInd = bidInd;
		this.trickInd = trickInd - App.deal.getPresetTrickCount();
...
public class DePointAy extends ArrayList<DePoint> {
	...
	public DePointAy(float width, float LINE_END_GAP) {
		...
		int pre_tricks = App.deal.preset_trick_count_x + App.deal.preset_trick_count_y;
		int pre_cards = pre_tricks * 4;

		int tricks = 0;
		int cards = 0;

		for (int i = 0; i < pre_cards + playedCards + 1; i++) {
			tricks = i / 4;
			cards = i % 4;
			...
			if (i > pre_cards)
				state = 'h'; // has happened

			if (App.isMode(Aaa.REVIEW_PLAY)) {
				if (i > pre_cards + reviewCard)
					state = 'u';
			}
			...
			if (i == 52) {
				break; // the End blob is always added at the end so we don't duplicate it here
			}
		...
		for (int i = pre_cards + playedCards + 1; i < 52; i++) { // 1 - 52 but 52 is the fin blob
			char stateM = 'm';
			int m_tricks = i / 4;
			int m_cards = i % 4;
			boolean spike = false;
