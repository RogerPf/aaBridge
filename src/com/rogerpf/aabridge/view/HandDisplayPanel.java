...
public class HandDisplayPanel extends ClickPanel { // ============ HandDisplayPanel
	...
	public void startDrag(Card card) {
		...
		char c = Rank.rankToLanguage(card.rank.toChar(), '-'/*, false  use_xes_char */);
	...
	private class MouseListener extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (/*App.isMode(Aaa.EDIT_PLAY) && */ !floatingHand && youDisplayRect.contains(e.getPoint())) {
				if (App.youSeatHint == hand.compass && deal.youSeatHint == hand.compass || App.isStudyDeal()) {
					return; // no change
				}
	...
	public String addTransformationsAndPadding(String cards, char showXes, int[] leftpos) {
		 ...
		 * < and >  are 300 but with tiny dot (invisible) in the left and right corners respectively
		...
		for (int i = 0; i < len; i++) {
			...
			/*
			 *  Is this card to be converted to an x ?
			 */
//			if (showXes != '-') {			
//				if (showXes == 'x') {
//					c = 'x';
//				}
//				else if (showXes == '*') {   // * asterisk = hide blot out
//					c = ' ';
//				}
//				else {	
//					int xv  = 0;
...
//			}		
			 ...
			 */
			char conv_c = Rank.rankToLanguage(c, showXes);
	...
	/**
	 */
	private Frag[] getAppropriateFrags(boolean actual_display) {
		// =============================================================
		...
		{			
			if (actual_display && !App.isVmode_InsideADeal()/* && App.isMode(Aaa.NORMAL_ACTIVE) */) {
				
				return hand.fragsfOrgsVisFeintMerge(); // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	*/
	...
	public void paintComponent(Graphics g) {
		...
		// --------------------------------------------------
		if (floatingHand) {
			if (App.mg.fillFloatingHandBackground_painttime) {
			...
			dummy = deal.isContractReal() && hand.isDummy();

			if (App.isVmode_InsideADeal()) {
				dummy = dummy && (App.isMode(Aaa.REVIEW_PLAY) || App.isMode(Aaa.NORMAL_ACTIVE) || App.isMode(Aaa.EDIT_PLAY));
			}

			g2.setColor(dummy ? Aaa.handBkColorDummy : Aaa.handBkColorStd);
			g2.fill(rd);
		...
		Color displayedWordColor = dummy ? Aaa.veryWeedyBlacHid : Aaa.veryWeedyBlack;

		Dir youSeat = hand.compass; // safety default for floating hands

		if (!floatingHand) {

			boolean dummyOk = App.isMode(Aaa.REVIEW_BIDDING);

			youSeat = deal.getTheYouSeat(dummyOk);
			boolean youSeatUs = (youSeat == hand.compass);
			...
			youDisplayRect = new Rectangle2D.Float(boarderThickness, boarderThickness, dealLozengeWidth, youLozengeHeight);
			...
			boolean displayMetrics = !deal.dfcDeal && visSeat && hand.didHandStartWith13Cards() && !deal.isDoneHand() && !sd;
			boolean displayHCPs = displayMetrics && App.showHCPs;
			boolean display2ndMetric = displayMetrics && (App.show2ndMetric != App.Metric_None) && !sd;

			// The "You" text and name
			// ------------------------------------------------------------------
			float yAdj = 0.226f;
			if (youSeatUs && (hand.playerName.isEmpty() || App.isMode(Aaa.EDIT_PLAY) || App.isMode(Aaa.EDIT_HANDS))) {
			...
			else if (App.isMode(Aaa.EDIT_PLAY) || App.isMode(Aaa.EDIT_HANDS)) {
...
//
		...
		Frag[] frags = getAppropriateFrags(true /* actual_display */); // <<<<<<<<<<<<<<<<<<<<<<<<<<<< Frags <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

		boolean ddsInUse = false;

		// @formatter:off
		if ((App.ddsDeal == null) 
				&& App.haglundsDDSavailable 
				&& App.ddsScoreShow 
				&& App.isPauseAtEotClickWanted() 
				&& App.isAutoPlay(hand.compass)
				&& App.deal.isCurTrickComplete()
		...
		Boolean suppress_displayedWordInBox = false;

		Boolean xesUseVaild = App.deal.showXesValidInsideDeal || (App.isVmode_InsideADeal() == false);
		...
		for (Frag frag : frags) {
			...
			char showXes = xesUseVaild ? frag.showXes : '-';

			if (showXes == '*' && App.alwaysShowHidden /* && App.devMode */) {
				showXes = '-'; // override
			}
			...
			String rawDispEx = frag.toDisplayStr();
			...
			// Cards of the frag and individual pap Visibility of the suit

			if (deal.dfcDeal && ((frag.suitVisControl & Suit.SVC_cards) != Suit.SVC_cards) /* the dfc hide suit mechanism */) {
				continue;
			}
			else if (!visSeat) {
				if ((App.papBits & frag.suit.get_pap()) == 0) {
					continue;
				}

				if (hand.compass != youSeat.rotate(App.pap_who)) {
					continue;
				}

				suppress_displayedWordInBox = true;
				if (App.papXes) {
					cards = addTransformationsAndPadding(rawCards, 'x', ddsRightPos);
				}
			}
			...
			for (int i = 0; i < cards.length(); i += 3) {
				...
				char c = rawDispEx.charAt(i / 3);
				if (c == 'p' || c == 'd') {
					Color col;
					if (c == 'p')
						col = dummy ? Aaa.playedCardBlackDum : Aaa.playedCardBlack;
					else
						// 'd'
						col = frag.suit.colorCd(Cc.Ce.Weedy);
					astr.addAttribute(TextAttribute.FOREGROUND, col, i + 1, i + 2);
				}
			}
			...
			if (ddsInUse) {
				...
				float fontScale = (App.showDdsScore_aaB_style ? 0.55f : 0.48f);
...
//				float dotDiameter = (float) ddsCharWidth * 0.55f;
				...
				int weWonSoFar = App.ddsDeal.getContractTrickCountForDirecton(hand.compass);

//				int declarerWonSoFar = App.ddsDeal.getContractTrickCountForDirecton(deal.contractCompass);

				boolean ourContract = (deal.contractCompass == hand.compass) || dummy;
				int contractTarget = deal.contract.level.v + 6;
				...
				for (int i = 0; i < totalCards; i++) {
					...
					// @formatter:off
					double w =  ddsCharWidth  * (App.showDdsScore_aaB_style ? 1.10 : 1.0 );
					double wR = ddsCharWidth  * (App.showDdsScore_aaB_style ? 1.10 : 0.3 );
					double w2 = ddsCharWidth  * (App.showDdsScore_aaB_style ? 0.35 : 0.20);
					double h =  ddsCharHeight * (App.showDdsScore_aaB_style ? 1.10 : 0.92);
					double hR = ddsCharHeight * (App.showDdsScore_aaB_style ? 1.10 : 0.5 );
					// @formatter:off
					...
					if (App.showDdsScore_aaB_style) {
						score = ddsScore + weWonSoFar;
						backGround = (((ddsScore == highestDdsScore) ? Aaa.scoreaaBBest : Aaa.scoreaaBOthr));
						fontColor = Cc.BlackStrong;
					}
					else // use the 'BBO' style
					{
						if (ourContract) {
							score = (ddsScore + weWonSoFar) - contractTarget;
						}
						else {
							score = (13 - (ddsScore + weWonSoFar) - contractTarget);
						}
					...
					RoundRectangle2D ddsBackground = new RoundRectangle2D.Double((double) x + w2, y - h, w, h, wR, hR);
					g2.setColor(backGround);					
					g2.fill(ddsBackground);

					g2.setColor(fontColor); // digits and symbols are shown in black or white

					TextLayout text = new TextLayout(card.getDisplayScoreDispChar(score, App.showDdsScore_aaB_style), ddsScoreFont, frc);
		...
		if (!floatingHand && !visSeat && ! suppress_displayedWordInBox) {
			g2.setFont(BridgeFonts.internationalFont.deriveFont(handFontSize));
			g2.setColor(displayedWordColor);
			g2.drawString(displayedWordInBox, dealLozengeWidth * (deal.isDoneHand() ? 0.15f : 0.3f), dealLozengeHeight * 0.650f);
		}
