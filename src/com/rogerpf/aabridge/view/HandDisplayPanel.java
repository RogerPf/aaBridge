...
//
		...
		for (Frag frag : frags) {
			...
			if (ddsInUse) {
				...
				float fontScale = (App.showDdsScore_aaB_style ? 0.55f : 0.55f);
...
//				float dotDiameter = (float) ddsCharWidth * 0.55f;
				...
				int ddsHighestScore = App.ddsDeal.getHighestDdsScore();
...
//				int declarerWonSoFar = App.ddsDeal.getContractTrickCountForDirecton(deal.contractCompass);
				...
				for (int i = 0; i < totalCards; i++) {
					...
					double w2 = ddsCharWidth  * (App.showDdsScore_aaB_style ? 0.25 : 0.20);
					double h =  ddsCharHeight * (App.showDdsScore_aaB_style ? 1.10 : 0.92);
					double hR = ddsCharHeight * (App.showDdsScore_aaB_style ? 1.10 : 0.0 );
					// @formatter:off
					...
					g2.setStroke(new BasicStroke((float)ddsCharWidth * 0.03f));
