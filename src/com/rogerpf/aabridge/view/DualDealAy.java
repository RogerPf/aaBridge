...
public class DualDealAy extends ArrayList<DualDeal> {
	...
	public boolean oneColExtended = false; // until we find otherwise
...
//					closed_count++;
	...
	public void afterGiAyCreated_constructor_part2(MassGi mg) {
		...
		for (DualDeal dd : this) {
			...
			if (openDeal.scoreValid && closedDeal.scoreValid) {
				...
				dd.impsValid = false;
				dd.sImps = "";
				if (!dd.travScore.isEmpty()) {
					dd.sImps = fakeRightJustify(dd.travScore);
				}
			}
		}
	...
	/**
	 */
	public String fakeRightJustify(String orig) {
		// ============================================================================
		int n = 5;
		String s = String.format("%" + n + "s", orig);
		if (orig.length() <= 4) {
			s = " " + s;
		}
		if (orig.length() <= 5) {
