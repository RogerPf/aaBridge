...
public class BottomLeftPanel extends ClickPanel {
	...
	public void matchPanelsToDealState() {
		...
		if (App.isVmode_Tutorial()) {
			...
			c0_2_0__empt.setVisible(false);
//			boolean xx = App.deal.fewerThanTwoCardsPlayed();
//			boolean yy = (!App.isAutoPlay(App.deal.getNextHandToPlay().compass));
			boolean showMsg = (!App.isAutoPlay(App.deal.getNextHandToPlay().compass)) && !App.isMode(Aaa.EDIT_PLAY) && App.showBidPlayMsgs
					&& App.deal.fewerThanTwoCardsPlayed();
			c0_2_0__clam.setVisible(!showMsg);
	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int i = 0; i < claimValBtns.length; i++) {
			if (claimValBtns[i] == source) {
				...
				// ourScore += (App.deal.isYouSeatDeclarerAxis() ? App.deal.preset_trick_count_x : App.deal.preset_trick_count_y);
				// int remainingTricks = 13 - (score.x + score.y);
...
class NextBoardButtonPanel extends ClickPanel {
	...
	NextBoardButtonPanel() { /* Constructor */
		...
		setLayout(new MigLayout(App.simple, "[]push[]7%[]", "[]10%"));

		RpfResizeButton b;

		b = new RpfResizeButton(Aaa.s_Std, "mainNewBoard", 40, 85, 0.75f);
		b.setForeground(Cc.RedStrong);
		add(b, "wmin 10%");
		mainNewBoard = b;

		b = new RpfResizeButton(Aaa.s_Std, "mainUndoBot", 16, 78);
		add(b, "");
		mainUndoBot = b;

		b = new RpfResizeButton(Aaa.s_Std, "mainUndoStandard", 32, 85);
		add(b, "");
		mainUndoStandard = b;
	...
	public void set_NextBoard_and_Undo_visibility() {
		...
		boolean en = !App.isStudyDeal() || App.sd_dev_visibility;

		mainUndoBot.setVisible(en && App.isVmode_InsideADeal() && ( /* App.isMode(Aaa.EDIT_PLAY) || */ App.isMode(Aaa.NORMAL_ACTIVE) && App.deal.isPlaying()));
		mainUndoStandard.setVisible(App.isVmode_InsideADeal() && (App.isModeAnyReview() == false));
	}
