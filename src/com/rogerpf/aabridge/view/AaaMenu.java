...
//		menu.add(menuItem);
	...
	public static JMenu makeHelpMenu(ActionListener listener, String menuBarText) {
		...
		menuItem.setActionCommand("openPage_OCPQuickTables");
		menuItem.addActionListener(listener);
		menu.add(menuItem);

		// OCP_website
		menuItem = new JMenuItem(Aaf.spacedOut(menuItem, ocp, firstPartLen) + Aaf.gT("helpMenu.ocpWebsite") + "                 " + web);
		menuItem.setActionCommand("web_OCPWebsite");
		menuItem.addActionListener(listener);
		menu.add(menuItem);
...
//		menu.add(menuItem);
		...
		menuItem.setActionCommand("openPage_Deal_and_Merge");
		menuItem.addActionListener(listener);
		menu.add(menuItem);
...
//		menu.add(menuItem);
        ...
        // What goes on inside a .lin file
		Bookshelf.addFirstShelf_90s_toMenu(listener, menu);

		menu.addSeparator();

		// aaBridge on Google Drive
//		String relArcOn = Aaf.gT("helpMenu.relArcOn");
		menuItem = new JMenuItem(Aaf.spacedOut(menuItem, Aaf.gT("helpMenu.website"), firstPartLen)  + "aaBridge Releases  from  Google Drive");
		menuItem.setForeground(Cc.GreenStrong);
		...
		// Facebook
		menuItem = new JMenuItem(Aaf.spacedOut(menuItem, "", firstPartLen) + "aaBridge  on  Facebook");
		menuItem.setActionCommand("web_aaBridgeFacebook");
		menuItem.addActionListener(listener);
		menu.add(menuItem);

		boolean show_OWS = true;
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            Date today = new Date();     	
			Date no_show_after = sdf.parse("2025-12-31");
			show_OWS = today.before(no_show_after);
		} catch (ParseException e) {
		}
  
		// Help Old Original Website
        if (show_OWS) {
        	menuItem = new JMenuItem(Aaf.spacedOut(menuItem, "", firstPartLen) + "Original aaBridge  website");
        	menuItem.setActionCommand("web_OldOriginalWebsite");
        	menuItem.addActionListener(listener);
        	menu.add(menuItem);
        }
