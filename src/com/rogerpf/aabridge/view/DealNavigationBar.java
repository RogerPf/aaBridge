...
//		}
		...
		for (int i = 0; i < App.dePointAy.size(); i++) {
			...
			if (dp.to > x || last) {
				...
				if (dp.type == 'b') {
					if (App.isStudyDeal())
						return; // bidding is closed off from us 
				...
				if (dp.type == 'c') {
					...
					CmdHandler.matchDealBkToReview();
					break;
				}
...
//		Color backgroundColor = entered ? Aaa.baizeGreenNav : Cc.g(Cc.baizeGreen);
		...
		for (int i = 0; i < App.dePointAy.size(); i++) {
			...
			// @formatter:off
			switch (dp.state) {
 			    case 'h': { g2.setColor(playedColor);  break; }
                case 'm': { g2.setColor(claimedColor); break; }
                case 's': { g2.setColor(Aaa.navPresetPlay);  break; }
                case 'u': { g2.setColor(unplayedColor); break; }
                default:    {
                	g2.setColor(Aaa.baizeGreenNav);  // to pick up any mis-settings
                }
            }
			// @formatter:on
