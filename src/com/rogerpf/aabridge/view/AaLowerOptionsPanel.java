...
class AaLowerOptionsPanel extends ClickPanel implements ChangeListener, ActionListener, ItemListener {
	...
	QCheckBox prob_force_comments;
	QCheckBox linSaveRotateDecSouth;
	QCheckBox includeRotationsSetBelow;
	QCheckBox multiSaveBboTourny;
	QCheckBox saveNoBidOrPlay;
	QCheckBox saveOnlyTheLead;
	QCheckBox discardPlayerNames;
	QCheckBox prependYyySavePrefix;
	QCheckBox alsoDealerSouth;
	JLabel labRot1;
	JLabel labRot2;
	...
	public void showButtonStates() {
		...
		multiSaveBboTourny.setEnabled(en);
		saveNoBidOrPlay.setEnabled(en);
		saveOnlyTheLead.setEnabled(en);
		discardPlayerNames.setEnabled(en);

		prependYyySavePrefix.setEnabled(en);
		alsoDealerSouth.setEnabled(en);
	}
	...
	public AaLowerOptionsPanel() {
		...
		b1.add(pbnInfoFirst = new QCheckBox(this, App.pbnInfoFirst, Aaf.gT("botPanel.pbnInfoFirst"), Aaf.gT("botPanel.pbnInfoFirst_TT")), ", gapy 0, gapx 2");
...
//		------- next column ---------------------------
		...
		b2.add(bboUpStripped = new QCheckBox(this, App.bboUpStripped, Aaf.gT("botPanel.bboUpStripped"), ""), "gapy0, gapx 10");
		...
		b2.add(linfileSaveFormat_2 = new QRadioButton(this, linGroup, null, App.linfileSaveFormat == 2, "", Aaf.gT("botPanel.linFmt_Prob")),
				"split 2, flowx, gapx 0");

		b2.add(prob_force_comments = new QCheckBox(this, App.prob_force_comments, "", Aaf.gT("botPanel.prob_force_comments_TT")), "gapy 5, gapx 7");
		prob_force_comments.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 0));
		...
		//  RE-NUMBER
		b2.add(renumberDealsLin = new QCheckBox(this, App.renumberDealsLin, Aaf.gT("botPanel.renumberOther"), Aaf.gT("botPanel.renumberOther_TT")),
				"gapy 3,  gapx 6, split 3, flowx");
		if (App.onMac == false) {
		...
		b2.add(prependYyySavePrefix = new QCheckBox(this, App.prependYyySavePrefix, Aaf.gT("botPanel.incYyySavePrefix"),
				Aaf.gT("botPanel.incYyySavePrefix_TT")), "gapx 10");
		prependYyySavePrefix.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			prependYyySavePrefix.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		//  Tourney multi save only force duplicate board numbers and vuln
		b2.add(multiSaveBboTourny = new QCheckBox(this, App.multiSaveBboTourny, Aaf.gT("botPanel.multiSaveBboTourny"),
				Aaf.gT("botPanel.multiSaveBboTourny_TT")), "gapx 6, split 4, flowx");
		// multiSaveBboTourny.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			multiSaveBboTourny.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		// dpn discardPlayerNames
		b2.add(discardPlayerNames = new QCheckBox(this, App.discardPlayerNames, Aaf.gT("botPanel.discardPlayerNames"),
				Aaf.gT("botPanel.discardPlayerNames_TT")), "gapx 12");
		// discardPlayerNames.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			discardPlayerNames.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		// min saveOnlyTheLead
		b2.add(saveOnlyTheLead = new QCheckBox(this, App.saveOnlyTheLead, Aaf.gT("botPanel.saveOnlyTheLead"), Aaf.gT("botPanel.saveOnlyTheLead_TT")),
				"gapx 12");
		// saveOnlyTheLead.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			saveOnlyTheLead.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		// nbp saveNoBidOrPlay
		b2.add(saveNoBidOrPlay = new QCheckBox(this, App.saveNoBidOrPlay, Aaf.gT("botPanel.saveNoBidOrPlay"), Aaf.gT("botPanel.saveNoBidOrPlay_TT")),
				"gapx 12");
		// saveNoBidOrPlay.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			saveNoBidOrPlay.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		//  Rotate DECLARER to South
		b2.add(linSaveRotateDecSouth = new QCheckBox(this, App.linSaveRotateDecSouth, Aaf.gT("botPanel.linSaveRotateDecSouth"),
				Aaf.gT("botPanel.linSaveRotateDecSouth_TT")), "gapx 6, flowx, split 2");
		if (App.onMac == false) {
			linSaveRotateDecSouth.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		//  Dealer (alsoDealerSouth)
		b2.add(alsoDealerSouth = new QCheckBox(this, App.alsoDealerSouth, Aaf.gT("botPanel.alsoDealerSouth"), Aaf.gT("botPanel.alsoDealerSouth_TT")),
				"gapx 10, flowy");
		// alsoDealerSouth.setForeground(Aaa.navDarkText);
		if (App.onMac == false) {
			alsoDealerSouth.setBorder(BorderFactory.createEmptyBorder(6, 0, 0, 0));
		}

		// INCLUDE ROTATIONS SET BELOW
	...
	void setRotateEnDis() {
		if (App.includeRotationsSetBelow) {
			if (App.linfileSaveFormat != App.linFmt_BBO) {
				App.linfileSaveFormat = App.linFmt_BBO;
				linfileSaveFormat_0.setSelected(true);
			}
//			App.multiSaveBboTourny = false;
//			multiSaveBboTourny.setSelected(false);
		}
...
//		}
		...
		boolean Std = App.linfileSaveFormat == App.linFmt_Std;

		bboUpStripped.setEnabled(BBO || Std);
		multiSaveBboTourny.setEnabled(BBO || Std); // && !App.linSaveRotateDecSouth && !App.includeRotationsSetBelow);
		discardPlayerNames.setEnabled(BBO || Std);
		saveNoBidOrPlay.setEnabled(BBO || Std);
		saveOnlyTheLead.setEnabled(BBO || Std);
		prob_force_comments.setEnabled(!(BBO || Std));

		if (!(BBO || Std)) {
			App.bboUpStripped = false;
			App.multiSaveBboTourny = false;
			App.saveNoBidOrPlay = false;
			App.saveOnlyTheLead = false;
			App.discardPlayerNames = false;
		}

		bboUpStripped.setSelected(App.bboUpStripped);
		multiSaveBboTourny.setSelected(App.multiSaveBboTourny);
		saveNoBidOrPlay.setSelected(App.saveNoBidOrPlay);
		saveOnlyTheLead.setSelected(App.saveOnlyTheLead);
		discardPlayerNames.setSelected(App.discardPlayerNames);

		alsoDealerSouth.setEnabled(App.linSaveRotateDecSouth);

		if (App.linSaveRotateDecSouth == false) {
			App.alsoDealerSouth = false;
			alsoDealerSouth.setSelected(false);
		}

		boolean b = App.includeRotationsSetBelow;

//		if (App.includeRotationsSetBelow) {
//			App.linSaveRotateDecSouth = false;
//			linSaveRotateDecSouth.setSelected(false);
//		}
//		if (App.linSaveRotateDecSouth) {
//			App.includeRotationsSetBelow = false;
//			includeRotationsSetBelow.setSelected(false);
//		}
...
//	}
	...
	public void itemStateChanged(ItemEvent e) {
		...
		else if (source == prob_force_comments) {
			App.prob_force_comments = b;
		}		
		...
		else if (source == linSaveRotateDecSouth) {
			App.linSaveRotateDecSouth = b;
		}
		...
		else if (source == multiSaveBboTourny) {
			App.multiSaveBboTourny = b;
		}
		else if (source == discardPlayerNames) {
			App.discardPlayerNames = b;
		}
		else if (source == saveNoBidOrPlay) {
			App.saveNoBidOrPlay = b;
			if (b) {
				App.saveOnlyTheLead = false;
			}
		}	
		else if (source == saveOnlyTheLead) {
			App.saveOnlyTheLead = b;
			if (b) {
				App.saveNoBidOrPlay = false;
			}
		}
		...
		else if (source == alsoDealerSouth) {
			App.alsoDealerSouth = b;
		}
		else if (source == pbnInfoFirst) {
			App.pbnInfoFirst = b;
		}
