...
public class GreenBaizePanel extends ClickPanel {
	...
	public void matchPanelsToDealState() {
		...
		if (App.isMode(Aaa.EDIT_HANDS)) {
			...
			boolean allCardsDealt = !App.deal.isIncomplete(); // isIncomplete => 1 to 51 cards delt so far
			boolean showCentralBidding = (tutorial && App.tutorialShowAuction) && allCardsDealt;
			boolean showCardFragments = !showCentralBidding && !allCardsDealt;

			c1_1__empt.setVisible(false);
			c1_1__bfdp.setVisible(showCentralBidding || !showCardFragments);
			c1_1__tfdp.setVisible(showCardFragments);
		...
		App.frame.rop.update_rightTabbed_outer_panels();
	}
