...
public class DualDeal {
	...
	public class EachDeal {
		...
		public AttributedString makeAttributedStringFromContract(boolean showResults) {
			...
			else if (contractDblRe.isReDouble()) {
				s += "r";
			}
			else {
				s += " ";
			...
			at = new AttributedString(s);
			Font bfont = (contract.suit == Suit.NoTrumps) ? BridgeFonts.faceAndSymbolFont.deriveFont(fontSize)
					: BridgeFonts.faceAndSymbolFont.deriveFont(fontSize);
